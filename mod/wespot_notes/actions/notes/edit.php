...
$variables = elgg_get_config('notes');
$input = array();
...
elgg_make_sticky_form('notes');

if (!$input['title']) {
	register_error(elgg_echo('notes:error:no_title'));
	forward(REFERER);
}
...
if ($page_guid) {
	...
	if (!$page || !$page->canEdit()) {
		register_error(elgg_echo('notes:error:no_save'));
		forward(REFERER);
	}
	$new_page = false;
} else {
...
if ($page->save()) {
	...
	elgg_clear_sticky_form('notes');

	// Now save description as an annotation
	$page->annotate('notes', $page->description, $page->access_id);

	system_message(elgg_echo('notes:saved'));

	if ($new_page) {
		add_to_river('river/object/notes/create', 'create', elgg_get_logged_in_user_guid(), $page->guid);
	}

	forward($page->getURL());
} else {
	register_error(elgg_echo('notes:error:notsaved'));
	forward(REFERER);
}

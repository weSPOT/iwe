...
//    }
  ...
  public function start_playing() {
    ...
    if (!array_key_exists($this->guid, $challenge_being_played)) {
      ...
      $randon_keys = array_rand($all_questions_array, $this->total_questions);
  ...
  public function save_results($complete_status = true) {
    ...
    $result->owner_guid = elgg_get_logged_in_user_guid();
    $result->annotate('izap_challenge_results', '', $result->access_id, $result->owner_guid);
    ...
    $result->save();
    add_to_river('river/object/izapchallengeresults/create', 'create', elgg_get_logged_in_user_guid(), $result->guid);

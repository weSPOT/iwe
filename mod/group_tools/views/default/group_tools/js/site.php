...
elgg.group_tools.duplicate_confirm = function(){
	return confirm(elgg.echo("group_tools:duplicate:confirm"));
}
...
elgg.group_tools.init = function(){
	...
	// duplicate group
	$('#group_tools_duplicate_form').submit(elgg.group_tools.duplicate_confirm);
	...
	// export popup box
	if($("a.group-tools-lightbox").length){
		$("a.group-tools-lightbox").fancybox({
			titleShow: false
		});
	}
	
	$("#group-tools-export-form .elgg-button").live("click", function(){
		$.fancybox.close();
	});

...
if (elgg_get_page_owner_entity()->canWriteToContainer())
{
    $new_link = $phase ?  elgg_view('output/url', array(
        'href' => "reflection/add/".$vars['entity']->owner_guid . "?phase=" . $phase . '&activity_id=' . $activity_id,
        'text' => elgg_echo('reflection:add'),
        'is_trusted' => true,
    )) : "Refresh to add reflection";
}

if ($content) {
	$url = "reflection/group/" . elgg_get_page_owner_entity()->getGUID();
	$more_link = elgg_view('output/url', array(
    'href' => $url . "?phase=" . $phase . '&activity_id=' . $activity_id,
		'text' => elgg_echo('reflection:more'),
		'is_trusted' => true,
	));
    if($new_link != "") { $new_link = ' | ' . "<span>" . $new_link . "</span>"; }
	echo "<span class=\"elgg-widget-more\">$more_link</span>" . $new_link;
} else {
	echo elgg_echo('reflection:none');
    echo "<br><br>";
    echo $new_link;
}

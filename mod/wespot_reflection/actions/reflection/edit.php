...
$variables = elgg_get_config('reflection');
$input = array();
...
elgg_make_sticky_form('reflection');

if (!$input['title']) {
	register_error(elgg_echo('reflection:error:no_title'));
	forward(REFERER);
}
...
if ($page_guid) {
	...
	if (!$page || !$page->canEdit()) {
		register_error(elgg_echo('reflection:error:no_save'));
		forward(REFERER);
	}
	$new_page = false;
} else {
	$page = new ElggObject();
	if ($parent_guid) {
		$page->subtype = 'reflection';
	} else {
		$page->subtype = 'reflection_top';
	}
  $page->phase = $phase;
...
if ($page->save()) {
	...
	elgg_clear_sticky_form('reflection');

	// Now save description as an annotation
	$page->annotate('reflection', $page->description, $page->access_id);

	system_message(elgg_echo('reflection:saved'));

	if ($new_page) {
		add_to_river('river/object/reflection/create', 'create', elgg_get_logged_in_user_guid(), $page->guid);
	}

	forward($page->getURL());
} else {
	register_error(elgg_echo('reflection:error:notsaved'));
	forward(REFERER);
}

...
if (elgg_instanceof($page, 'object', 'reflection') || elgg_instanceof($page, 'object', 'reflection_top')) {
	...
	if (elgg_is_admin_logged_in() || elgg_get_logged_in_user_guid() == $page->getOwnerGuid()) {
		...
		if ($children) {
			...
			$subtype_id = (int)get_subtype_id('object', 'reflection_top');
    ...
    $activity_id = $page->activity_id;
		...
		if ($page->delete()) {
			system_message(elgg_echo('reflection:delete:success'));
      elgg_trigger_event('delete', 'annotation_from_ui', $page);
			...
			if (elgg_instanceof($container, 'group')) {
				forward("reflection/group/$container->guid/all?phase=".$phase . '&activity_id=' . $activity_id);
			} else {
				forward("reflection/owner/$container->username");
			}
		}
	}
}

register_error(elgg_echo('reflection:delete:failure'));
forward(REFERER);

<?php
$language = array (
  'messages' => 'Μηνύματα',
  'messages:unreadcount' => '%s μη αναγνωσμένα',
  'messages:back' => 'επιστροφή στα μηνύματα',
  'messages:user' => 'Εισερχόμενα του χρήστη %s',
  'messages:posttitle' => '%s\' μηνύματα: %s',
  'messages:inbox' => 'Εισερχόμενα',
  'messages:send' => 'Αποστολή',
  'messages:sent' => 'Στάλθηκε',
  'messages:message' => 'Μήνυμα',
  'messages:title' => 'Θέμα',
  'messages:to' => 'Προς',
  'messages:from' => 'Από',
  'messages:fly' => 'Αποστολή',
  'messages:replying' => 'Μήνυμα απάντηση σε',
  'messages:sendmessage' => 'Αποστολή μηνύματος',
  'messages:compose' => 'Σύνθεση μηνύματος',
  'messages:add' => 'Σύνθεση μηνύματος',
  'messages:sentmessages' => 'Απεσταλμένα μηνύματα',
  'messages:recent' => 'Πρόσφατα μηνύματα',
  'messages:original' => 'Αρχικό μήνυμα',
  'messages:yours' => 'Το μήνυμα σας',
  'messages:answer' => 'Απάντηση',
  'messages:toggle' => 'Εναλλαγή όλων',
  'messages:markread' => 'Σημειώστε το ως διαβασμένο',
  'messages:recipient' => 'Επιλέξτε ένα παραλήπτη',
  'messages:to_user' => 'Προς: %s',
  'messages:new' => 'Νέο μήνυμα',
  'notification:method:site' => 'Ιστότοπος',
  'messages:error' => 'Παρουσιάστηκε πρόβλημα κατά την αποθήκευση του μηνύματος σας. Παρακαλούμε προσπαθήστε ξανά.',
  'item:object:messages' => 'Μηνύματα',
  'messages:posted' => 'Το μήνυμα στάλθηκε με επιτυχία.',
  'messages:success:delete:single' => 'Το μήνυμα διαγράφτηκε',
  'messages:success:delete' => 'Τα μηνύματα διαγράφτηκαν',
  'messages:success:read' => 'Τα μηνύματα χαρακτηρίστηκαν ως μη αναγνωσμένα',
  'messages:error:messages_not_selected' => 'Δεν επιλέχθηκαν μηνύματα',
  'messages:error:delete:single' => 'Αδύνατη η διαγραφή του μηνύματος',
  'messages:email:subject' => 'Έχετε ένα νέο μήνυμα!',
  'messages:email:body' => 'Έχετε ένα νέο μήνυμα από τον/την %s:

%s

Για να δείτε το μήνυμα σας, πατήστε εδώ:

%s

Για να στείλετε στον/στην %s ένα μήνυμα, πατήστε εδώ:

%s

Δεν μπορείτε να απαντήσετε σε αυτό το ηλεκτρονικό μήνυμα.',
  'messages:blank' => 'Πρέπει να γράψετε κάτι στο κυρίως κείμενο του μηνύματος προκειμένου να αποθηκευτεί.',
  'messages:notfound' => 'Δεν ήταν δυνατή η εύρεση του συγκεκριμένου μηνύματος.',
  'messages:notdeleted' => 'Δεν ήταν δυνατή η διαγραφή αυτού του μηνύματος.',
  'messages:nopermission' => 'Δεν έχετε δικαιώματα για να αλλάξετε αυτό το μήνυμα.',
  'messages:nomessages' => 'Δεν υπάρχουν μηνύματα.',
  'messages:user:nonexist' => 'Δεν βρέθηκε ο παραλήπτης στη βάση δεδομένων των χρηστών.',
  'messages:user:blank' => 'Δεν επιλέξατε κάποιο χρήστη για την αποστολή του μηνύματος',
  'messages:deleted_sender' => 'Διαγραμμένος χρήστης',
);
add_translation("el", $language);

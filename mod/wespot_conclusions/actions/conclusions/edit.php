...
$variables = elgg_get_config('conclusions');
$input = array();
...
elgg_make_sticky_form('conclusions');

if (!$input['title']) {
	register_error(elgg_echo('conclusions:error:no_title'));
	forward(REFERER);
}
...
if ($page_guid) {
	...
	if (!$page || !$page->canEdit()) {
		register_error(elgg_echo('conclusions:error:no_save'));
		forward(REFERER);
	}
	$new_page = false;
} else {
	$page = new ElggObject();
	if ($parent_guid) {
		$page->subtype = 'conclusions';
	} else {
		$page->subtype = 'conclusions_top';
	}
  $page->phase = $phase;
...
if(get_input('recommended_tags')) {
  $page->recommended_tags = get_input('recommended_tags');
}

if(get_input('tag_recommender_algorithm')) {
  $page->tag_recommender_algorithm = get_input('tag_recommender_algorithm');
}
...
if ($page->save()) {
	...
	elgg_clear_sticky_form('conclusions');

	// Now save description as an annotation
	$page->annotate('conclusions', $page->description, $page->access_id);

	system_message(elgg_echo('conclusions:saved'));

	if ($new_page) {
		add_to_river('river/object/conclusions/create', 'create', elgg_get_logged_in_user_guid(), $page->guid);
	}

	forward($page->getURL());
} else {
	register_error(elgg_echo('conclusions:error:notsaved'));
	forward(REFERER);
}

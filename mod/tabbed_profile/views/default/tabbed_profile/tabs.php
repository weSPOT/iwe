...
$counter = 0;
if ($profiles) {
  $phases = get_entity($profile->container_guid)->phases;

  foreach ($profiles as $profile) {
     if($phases === null || strpos($phases, (string)$profile->order) !== false) {
        $text = elgg_echo($profile->title);
        ...
        $counter++;
        if ($counter < $max_tabs && !$selected)
            $class = 'arrow_box';

        if($selected)
            $class = 'arrow_box_selected';

        if ($counter == $max_tabs)
            $class = 'arrow_box_last';

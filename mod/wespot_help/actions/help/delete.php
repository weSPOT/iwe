...
if (elgg_instanceof($page, 'object', 'help') || elgg_instanceof($page, 'object', 'help_top')) {
	...
	if (elgg_is_admin_logged_in() || elgg_get_logged_in_user_guid() == $page->getOwnerGuid()) {
		...
		if ($children) {
			...
			$subtype_id = (int)get_subtype_id('object', 'help_top');
		...
		if ($page->delete()) {
			...
			if (elgg_instanceof($container, 'group')) {
				forward("help/group/$container->guid/all");
			} else {
				forward("help/owner/$container->username");
			}
		}
	}
}

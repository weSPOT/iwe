...
//  add_group_tool_option('activity', elgg_echo('groups:enableactivity'), true);
  ...
  // Add Quizzes sidebar link
  elgg_register_plugin_hook_handler('register', 'menu:owner_block', 'groups_quizzes_sidebar');
 ...
 */
function groups_quizzes_sidebar($hook, $type, $return, $params) {
  ...
  if (is_group_member ( $group->guid, elgg_get_logged_in_user_guid () )) {
    ...
    if($quizzes) {
      ...
      if($quizzes)
        ...
        $item = new ElggMenuItem('groups_quizzes', elgg_echo('izap-contest:contests'), $url);
        $return[] = $item;
  }
  return $return;
}
...
function groups_icon_url_override($hook, $type, $returnvalue, $params) {
  ...
  if ($icontime) {
    // return thumbnail
    if (is_array($icontime)) {
      # error_log("DEBUG apparently icontime is an array...");
      # error_log(print_r($icontime, true));
      return "groupicon/".$group->guid."/".$size."/".$icontime[1].".jpg";
    } else {

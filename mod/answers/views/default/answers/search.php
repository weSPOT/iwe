...
if ( $keyword != 'false' ) {
	...
	if ( $owner_guid ) {
		...
		if ($owner->canWritetoContainer()) {
			...
			$html .= "<a style='margin-left: 10px' class='elgg-button elgg-button-action' href='" . elgg_get_site_url() . "answers/add/{$owner_guid}/?phase=" . $phase . "&activity_id=" . $activity_id . "&search={$html_keyword}'>" . elgg_echo('answers:add') . '</a>';
			echo $html . $content;
		} else {
			if ($content) {
				echo $content;

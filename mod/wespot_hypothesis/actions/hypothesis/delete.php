...
if (elgg_instanceof($page, 'object', 'hypothesis') || elgg_instanceof($page, 'object', 'hypothesis_top')) {
	...
	if (elgg_is_admin_logged_in() || elgg_get_logged_in_user_guid() == $page->getOwnerGuid()) {
		...
		if ($children) {
			...
			$subtype_id = (int)get_subtype_id('object', 'hypothesis_top');
    ...
    $activity_id = $page->activity_id;
		...
		if ($page->delete()) {
			system_message(elgg_echo('hypothesis:delete:success'));
      elgg_trigger_event('delete', 'annotation_from_ui', $page);
			...
			if (elgg_instanceof($container, 'group')) {
				forward("hypothesis/group/$container->guid/all?phase=" . $phase . '&activity_id=' . $activity_id);
			} else {
				forward("hypothesis/owner/$container->username");
			}
		}
	}
}

register_error(elgg_echo('hypothesis:delete:failure'));
forward(REFERER);

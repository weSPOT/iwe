...
elgg_make_sticky_form('hypothesis');

if (!$input['title']) {
	register_error(elgg_echo('hypothesis:error:no_title'));
	forward(REFERER);
}
...
if ($page_guid) {
	...
	if (!$page || !$page->canEdit()) {
		register_error(elgg_echo('hypothesis:error:no_save'));
		forward(REFERER);
	}
	$new_page = false;
} else {
	$page = new ElggObject();
	if ($parent_guid) {
		$page->subtype = 'hypothesis';
	} else {
		$page->subtype = 'hypothesis_top';
	}
    $page->phase = $phase;
...
if ($page->save()) {
	...
	elgg_clear_sticky_form('hypothesis');

	// Now save description as an annotation
	$page->annotate('hypothesis', $page->description, $page->access_id);

	system_message(elgg_echo('hypothesis:saved'));

	if ($new_page) {
		add_to_river('river/object/hypothesis/create', 'create', elgg_get_logged_in_user_guid(), $page->guid);
	}

	forward($page->getURL());
} else {
	register_error(elgg_echo('hypothesis:error:notsaved'));
	forward(REFERER);
}

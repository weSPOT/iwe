...
//Prevents Double submits, allows submit button value to be passed to action
//To also prevent users from submitting when JS is disabled, set "disabled" attribute to buttons in html/php
elgg.provide('elgg.buttons');
elgg.buttons.init = function(){
	$(".elgg-button[type=submit]").attr("disabled",false);
	$("form").delegate(".elgg-button[type=submit]", "click", function(e){
		$(this).parents('form').eq(0).submit();
		$(".elgg-button[type=submit]").each(function (index) {
			$(this).clone(false).removeAttr('name').prop('disabled', true).insertBefore($(this));
			$(this).hide();
		});
	});
	//return true;
}
elgg.register_hook_handler('init', 'system', elgg.buttons.init);
